...
- **Offen für 2b — entschieden und gebaut in 2b-1:** Durchgang A leert bei Lehrmaterial `geprueftVon` (Abschnitt „Der Compiler-Skill"), und schon das erste Einlesen legt einen Lehrplan ohne Freigabe an. Ohne eigenen Zustand verschwände die Karte samt Zahlen, und alle früher freigegebenen Lektionen der Quelle stünden als „ohne Lehrplaneintrag" da. Deshalb gibt es jetzt „wartet auf Freigabe": Fehlen nur `geprueftVon` und `geprueftAm`, bleibt die Karte mit ihren Zahlen stehen und trägt die Markierung. Siehe Nachtrag nach 2b-1.

## Nachtrag nach 2b-1 (2026-09-24)

2b ist in drei Teile geschnitten; die Reihenfolge hat der Nutzer am 2026-09-22 festgelegt: **2b-1** Foliensätze im Terminal einlesen (gebaut) → **2c** Compiler für Lehrmaterial → **2b-2** Bücher und EPUB → **2b-3** Formular und Dev-Endpunkte. So entstehen am schnellsten Lektionen aus der Vorlesung. Plan: `docs/superpowers/plans/2026-09-23-bibliothek-2b1-folien.md` mit Präzisierungen und Nachträgen; die Messung am Material: `docs/recherche/2026-09-22-foliensaetze-befund.md`.
...
- **Vor- und Nachprüfung:** `npm run pruefe-quelle -- --name <kurzname> --vor` verlangt Freigabe, einen Stand wie im Manifest, dieselben Abschnitte und Seitenbereiche und mindestens einen beauftragten Abschnitt; sie nennt je Abschnitt Rohdatei, Folienbereich und die Folienlisten. `--nach` verlangt: kein Abschnitt mehr `beauftragt`, jede Lektion da, der Wortlaut sauber, keine Prinzip-Id doppelt. Beide gleichen auch **den Lehrplan selbst** gegen die Rohdateien ab — er liegt im Git, die Folien nicht —, jeden Text samt Kommentaren, und beide verlangen, dass eine Lektion den `satz` ihres Prinzips als `prinzip` und denselben `vorbehalt` trägt; so fällt kein Vorbehalt still weg.
