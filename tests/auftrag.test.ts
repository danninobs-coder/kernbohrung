...
describe('werkzeug/auftrag.mjs direkt aufgerufen', () => {
  const SKRIPT = path.resolve(__dirname, '..', 'werkzeug', 'auftrag.mjs');

  // Mit node gestartet wie von npm run auftrag; erkennte das Werkzeug den
  ...
  it('zeigt ohne Argumente die Aufruf-Hilfe und endet mit 2', () => {
    const ordner = mkdtempSync(path.join(tmpdir(), 'kernbohrung-auftrag-direkt-'));
    try {
